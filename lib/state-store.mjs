 ...
 * 添加并行组到会话
 * @param {string} configName - 配置名称
 * @param {Object} groupInfo - 并行组信息
 * @returns {Object} 更新后的会话对象
 */
export function addParallelGroup(configName, groupInfo) {
  const sessionPath = getSessionPath(configName);
  const session = readSession(configName);
  ...
  // 初始化 parallelGroups 数组（向后兼容）
  if (!session.parallelGroups) {
    session.parallelGroups = [];
  }

  // 添加并行组信息
  session.parallelGroups.push({
    ...groupInfo,
 ...
 * @returns {Object} 更新后的会话对象
 */
export function updateParallelGroup(configName, groupId, updates) {
  const sessionPath = getSessionPath(configName);
  const session = readSession(configName);
  ...
  // 查找并更新并行组
  const groupIndex = session.parallelGroups.findIndex(g => g.id === groupId);

  if (groupIndex === -1) {
    throw new Error(`Parallel group not found: ${groupId}`);
  }

  session.parallelGroups[groupIndex] = {
    ...session.parallelGroups[groupIndex],
    ...updates,
 ...
 * @returns {Array} 并行组列表
 */
export function getParallelGroups(configName) {
  const session = readSession(configName);
