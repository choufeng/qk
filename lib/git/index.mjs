...
/**
 * Resolve git-relative file paths to absolute paths, filtered by cwd.
 * Returns objects with absolute path (for git add) and display path (for UI).
 *
 * @param {string} gitRoot - Absolute path to git repository root
 * @param {string[]} relativeFiles - File paths relative to git root
 * @param {string} cwd - Current working directory to filter by
 * @returns {Array<{absolute: string, display: string}>}
 */
export function resolveGitFiles(gitRoot, relativeFiles, cwd) {
  return relativeFiles
    .map(file => {
      const absolute = `${gitRoot}/${file}`
      return { absolute, file }
    })
    .filter(({ absolute }) => absolute.startsWith(cwd === gitRoot ? gitRoot : `${cwd}/`) || absolute === `${cwd}`)
    .map(({ absolute, file }) => {
      // Display path relative to cwd
      const cwdPrefix = cwd === gitRoot ? gitRoot : cwd
      const display = absolute.startsWith(`${cwdPrefix}/`)
...
/**
 * Get modified (unstaged) and untracked files in working tree.
 * Returns objects with absolute path (for git add) and display path (for UI),
 * filtered to only include files under the current working directory.
 *
 * @returns {Promise<Array<{absolute: string, display: string}>}
 */
export async function getModifiedFiles() {
  const [diffResult, untrackedResult, rootResult] = await Promise.all([
    $`git diff --name-only`,
  ...
  return resolveGitFiles(gitRoot, allFiles, cwd)
}
